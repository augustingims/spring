...
@ApiResponse(code = 500, message = "Failure")})
    ...
    @Path("/getCustomData")
    @Produces("application/json")
    @ApiOperation(value = "Retrieves a book based on ID.", response = DataDto.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Success", response = DataDto.class),
            @ApiResponse(code = 401, message = "Unauthorized"),
            @ApiResponse(code = 403, message = "Forbidden"),
            @ApiResponse(code = 404, message = "Not Found"),
@ApiResponse(code = 500, message = "Failure")})
	public List<DataDto> getCustomData() {
		return personneServiceImpl.getListeCostumData();
	}
}
